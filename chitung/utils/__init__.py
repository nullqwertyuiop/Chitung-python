        ...
        inline_dispatchers=[
            Twilight(
                [
                    ...
                    UnionMatch("global", "fish", "casino", "responder", "game", "lottery", optional=True) @ "funct"
                ]
            )
        ],
        decorators=[
...
async def chitung_group_config_handler(
    ...
    funct_type = {
        "global": "全局消息", "fish": "钓鱼", "casino": "娱乐游戏", "responder": "关键词触发功能", "game": "所有游戏",
        "lottery": "C4和Bummer功能"
    }
    if not funct.matched:
        await app.sendGroupMessage(
            event.sender.group,
            MessageChain("群设置指示词使用错误，使用/close或者/open加上空格加上global、game、casino、responder、fish或者lottery来开关相应内容。")
        )
        return
    funct = funct.result.asDisplay()
    value = option.result.asDisplay() == "open"
    gc = group_config.get(event.sender.group.id)
    exec(f"gc.{funct.replace('global', 'globalControl')} = {value}")
    save_group_config()
    await app.sendGroupMessage(
        event.sender.group,
